...
void Delay250ms(unsigned long msec);
...
void Delay1ms(unsigned long msec){
 unsigned long i;
  while(msec > 0){
    i = 133333;  // this number means 10ms
    while(i > 0){
      i = i - 1;
    }
    msec = msec - 1; // decrements every 10ms
  }
}
...
void Delay250ms(unsigned long msec){
 unsigned long i;
  while(msec > 0){
    i = 3333332;  // this number means 250ms
    while(i > 0){
      i = i - 1;
    }
    msec = msec - 1; // decrements every 250 ms
  }
}
